...
class Convolution(NodeOp):
    def __init__(self, data, weights, bias, node_name, pad='SAME', stride=None, group=1, dtype=FQDtype.FP32, branched=None):
        ...
        if pad == 'SAME':
           ...
           print("SAME convolution padding:",self.pad)
        # Padding
        elif pad == 'FULL':
             ih = self.data.shape[-3]
             iw = self.data.shape[-2]
             oh = ih/self.stride[-3]
             ow = iw/self.stride[-2]
             sh = self.stride[-3]
             sw = self.stride[-2]
             ...
             print("FULL Convolution Padding:",self.pad)
        elif pad == 'VALID':
            self.pad = ((0,0), (0,0), (0,0), (0,0))
            print("VALID Convolution Padding:",self.pad)
        else:
            assert len(pad) == 2
            self.pad = pad
    ...
    def __init__(self, data, pooling_kernel, node_name, pad='VALID', stride=None, dtype=None):
        ...
        if pad == 'VALID':
            self.pad = (
                    ...
                    (0,0),
                    (0,0))
            print("Maxpool VALID padding:",self.pad)
        elif pad == 'SAME':
            w = self.data.shape[-2]
            h = self.data.shape[-3]
            ...
            self.pad = (
                    ...
                    (pad_w_l,pad_w_r),
                    (0,0))
            print("Maxpool SAME padding:",self.pad)
        elif pad == 'FULL':
            ...
            sh = self.stride[-3]
            sw = self.stride[-2]
            kh = self.pooling_kernel[-3]
            kw = self.pooling_kernel[-2]
            ph = (oh - 1)* sh - ih + kh
            pw = (ow - 1)* sw - iw + kw
            ...
            print("Maxpool FULL padding:",self.pad)
        else:
            _pad = []
...
def conv2D(i, w, b, name=None, stride=None, pad='SAME', group=1, dtype=None, branched=False):
    g = get_default_graph()
    op = Convolution(i, w, b, name, stride=stride, pad=pad, group=group, dtype=dtype, branched=branched)
    return typecast(op.output_tensors, dtype)
...
def fc(i, w, b, name=None, stride=None, pad='SAME', group=1, dtype=None):
    ...
    wout = i.shape[-1]
    size = cout*hout*wout
    print('In FC. {}*{}*{}={}'.format(cout,hout,wout,size))
#    ip1=flatten(i,name,dtype)
    in_fc= get_tensor(shape=(1,1,1,size),name='fcin',dtype=dtype,trainable=False)    
    print ('in fc function, in_fc: {}'.format(in_fc))
    op = Convolution(in_fc, w, b, name, stride=stride, pad=pad, group=group, dtype=dtype)
    return typecast(op.output_tensors, dtype)
...
def flatten(i, name=None, dtype=None):
    g = get_default_graph()
    op = Flatten(i, name)
    print('Inside Flatten function, op tensors:{}'.format(op.output_tensors))
    print('OutputTensors_shape:{},{}'.format(op.output_tensors.shape[-2],op.output_tensors.shape[-1]))
    return typecast(op.output_tensors, dtype)
...
def create_flatten_layer(layer):
    ...
    return typecast(layer.output_tensors,dtype)
