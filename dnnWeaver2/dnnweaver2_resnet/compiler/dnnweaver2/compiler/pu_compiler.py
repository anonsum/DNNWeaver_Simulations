...
class PUCompiler(object):
    ...
    def compile_layer(self, conv_tiling, conv_op, pu_ops, simd_lanes=4):
        """
        ...
        if len(pu_ops) > 0:
            print('MS. In pu_compile. Alloc size: {}'.format(pu_ops[-1].output_tensors))
        ...
        oh = conv_tiling['OH/oh'][1] + pool_pad_h  #Padding added for overlapping strides 
        ow = conv_tiling['OW/ow'][1] + pool_pad_w  #Padding added for overlapping strides

        # get input tensor size
        B, OH, OW, OC = conv_op.output_tensors.shape ###This is input tensor fpga shape
        ...
        for op in pu_ops:
            if isinstance(op, BatchNorm):
                ...
                self.log.info('SS : In Layer :{}'.format(op.name))
                self.log.info('SS : Addr -- Mean : {}'.format(hex(op.mean.fpga_addr)))
                self.log.info('SS : Addr -- Scale : {}'.format(hex(op.scale.fpga_addr)))
            if isinstance(op, MaxPooling):
                pool_op = op
                pre_pool = False
            else:
        ...
        if len(pu_ops) > 0:
            ...
            t_out = conv_op.output_tensors

        if (conv_op.branched is True): 
            addr_offset = (t_out.shape[-1]) * (t_out.dtype.bits)/8
            if(self. concat_input_tensor == 0):
                print("Branch_1 node_name=",conv_op.name)
                self. concat_input_tensor = 1 
            elif (self.concat_input_tensor == 1):
                print("Branch_2 node_name=",conv_op.name)
                t_out = pu_ops[-1].output_tensors 
                t_out.fpga_addr += int(addr_offset)
                self.concat_input_tensor = 0
        t_out_addr = t_out.fpga_addr
        pad_offset = 0
        for i in range(len(t_out.shape)):
            pad_offset += t_out.fpga_pad[i][0] * np.prod(t_out.fpga_shape[i+1:])
        print('MS. Before t_out_addr: {}, len:{},pad_offset:{}'.format(hex(t_out_addr),len(t_out.shape),pad_offset))
        pad_offset = int(pad_offset * t_out.dtype.bits / 8)
        t_out_addr += pad_offset
        print('MS. FInal t_out_addr: {}, pad_offset:{}'.format(hex(t_out_addr),pad_offset))
...
###########MS
        ...
        #pu_inst_list.append(BaseAddressInstruction(1,0,t_out.fpga_addr))
        #pu_inst_list.append(BaseAddressInstruction(1,1,t_out.fpga_addr))
########
        if ld0_required:
        ...
        for loop, it in conv_tiling.items():
            if it[0] > 1 and loop in _pool_tile:
                if len(pu_ops) > 0:
                    ...
                    P_B,P_OH,P_OW, P_OC = conv_op.output_tensors.fpga_shape
