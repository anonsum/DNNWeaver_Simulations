...
def get_stats_fast(conv_params, tiling, order_type, verbose=False):
    ...
    """
    acc_obj, K, O, S, IC, OC, B, iprec, wprec, im2col, energy_cost, _, _,_ = conv_params
...
def optimize_for_order(conv_params, pool_kernel=None, pool_stride=None, sequential=True,pool_pad=None):
    # Generate permutations for the order
    loops = ['B/b', 'OW/ow', 'OH/oh', 'IC/ic', 'OC/oc']
    order = permutations(loops)
    return_dict = {}
    ...
    conv_params_with_pool = acc_obj, K, O, S, IC, OC, B, iprec, wprec, im2col, energy_cost, pool_kernel, pool_stride, pool_pad
...
def _optimize_for_order(conv_params, order_type, verbose=False):
    ...
    """
    acc_obj, K, O, S, IC, OC, B, iprec, wprec, im2col, energy_cost, pool_kernel, pool_stride, pool_pad = conv_params
    I = (O - 1) * S + K
    #print("Optimizer:Pool_kernel:{} pool_stride:{} pool_pad:{}".format(pool_kernel,pool_stride,pool_pad))
    #O_pad  = O + pool_pad[1][1]#FIX
    #pool_O = (O_pad - pool_kernel[1]) / pool_stride[1] + 1 #FIX1
    pool_O = (O - pool_kernel[1])/ pool_stride[1] + 1 #FIX2
    #print("Optimizer:O:{} K:{}, S:{} , IC;{}, OC:{}, B:{} Pool_output:{}".format(O,K,S,IC,OC,B,pool_O))
    ...
    for _b in range(num_B_tiles):
        ...
        for _o in range(num_O_tiles):
            ...
            p_oh = p_ow
            ow = (p_ow-1) * pool_stride[1] + pool_kernel[1] - pool_pad[1][1] #FIX
            oh = (p_oh-1) * pool_stride[2] + pool_kernel[2] - pool_pad[2][1] #FIX
            ...
            for _ic in range(num_IC_tiles):
                ...
                for _oc in range(num_OC_tiles):
                    ...
                    iw = K + (ow - 1) * S
                    ih = K + (oh - 1) * S
                    #print("oh:{} ow:{} ih:{} iw:{}".format(oh,ow,ih,iw))
                    tiling = {}
                    tiling['B/b'] = (num_b, b)
