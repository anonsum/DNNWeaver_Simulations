...
def genWtDataConv(sysarray, wt_list, tfd, tfh, qm, qn):
    OPch = len(wt_list)    
    M = sysarray.shape[0]
    N = sysarray.shape[1]
    K = wt_list[0].shape[1]
...
#def genWtDataFC0(sysarray, wt_list, tfd, tfh, qm, qn):
#    OPch = len(wt_list)    
#    M = sysarray.shape[0]
#    N = sysarray.shape[1]
#    IPch = wt_list[0].shape[0]
...
#           sysarray.fill(0) 
#
#def genWtDataFC0_3rd(sysarray, wt_list, tfd, tfh, qm, qn, conv_out_ch):
#    OPch = len(wt_list)    
#    M = sysarray.shape[0]
#    N = sysarray.shape[1]
#    IPch = wt_list[0].shape[0]
...
#        ochunks += 1
#    
#    ichunks = conv_out_ch/N
#    if conv_out_ch % N > 0:
#        ichunks += 1
#
#    data_chunks = IPch/conv_out_ch    
#   
#    print "OPch:", OPch, ", IPch:", IPch, ", conv_out_ch:", conv_out_ch, ", ochunks:", ochunks, ", ichunks:", ichunks, ", data_chunks:", data_chunks
#    for och in range(ochunks):
#        for ich in range(ichunks):
#            for d in range(data_chunks):
#                for m in range(M):
#                    if och * M + m < OPch:
#                        arr = wt_list[och * M + m]
#                        for n in range(N):
#                            start = data_chunks * (ich * N)
#                            idx = start + (data_chunks * n) + d
#                            if idx < IPch:
#                                print "och:", och, ", ich:", ich, ", d:", d, ", m:", m, ", n:", n, ", start:", start, ", idx:", idx, ", omat:", och * M + m
#                                val = arr[idx]
#                                sysarray[m][n] = val
#                t = np.transpose(sysarray)
#                c = np.reshape(t, M * N)
...
def genWtDataFC1(sysarray, wt_list, tfd, tfh, qm, qn):
    OPch = len(wt_list)
    M = sysarray.shape[0]
    N = sysarray.shape[1]
    IPch = wt_list[0].shape[0]
