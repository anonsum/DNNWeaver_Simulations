...
### Conv+conv+FC
    with graph.name_scope('inputs'):
        i = get_tensor(shape=(batch_size,8,8,3), name='data', dtype=FQDtype.FXP16, trainable=False)    #8x8 
    ...
    with graph.name_scope('conv1'):
        #weights = get_tensor(shape=(128, 3, 3, 3),
        weights1 = get_tensor(shape=(1,3,3, 1),
                             name='weights1',
                             dtype=FixedPoint(16,12))
        #biases = get_tensor(shape=(128),
    ...
    with graph.name_scope('fc0'):
        ...
        weights1 = get_tensor(shape=(6,1,1, size),
                             name='weights1',
                             dtype=FixedPoint(16,12))
        biases1 = get_tensor(shape=(6),
                             name='biases1',
                             dtype=FixedPoint(32,20))
    ...
    with graph.name_scope('fc1'):
        ...
        size = cout*hout*wout
        in_fc= get_tensor(shape=(1,1,1,size),name='fcin',dtype=FixedPoint(16,8),trainable=False)    
        weights1 = get_tensor(shape=(2,1,1, size),
                             name='weights1',
                             dtype=FixedPoint(16,12))
        biases1 = get_tensor(shape=(2),
                             name='biases1',
                             dtype=FixedPoint(32,20))
