...
class FPGAMemoryManager(object):
    ...
    def alloc_MS(self, tensor,flag):
        ...
        if tensor.fpga_addr is None:
            self.log.debug('MS: curr_ddr_ptr: {},prev_ddr_ptr:{}   fpga_size_in_bytes: {}'.format(self.curr_ddr_ptr,self.prev_ddr_ptr,tensor.fpga_size_in_bytes))
            if flag == 1 :
            ...
            self.curr_ddr_ptr += int(math.ceil(tensor.fpga_size_in_bytes / 1024) * 1024) + 2048
...
#        for tname, t in graph.tensor_registry.iteritems():
    ...
    def _conv_compile(self, conv_op, pu_op, tiling, array_n, array_m, last=False):
        ...
        self.log.info('MS : Addr -- Bias : {} \t Outputs : {} \t Elements : {}'.format(hex(conv_op.bias.fpga_addr),hex(conv_op.output_tensors.fpga_addr), conv_op.output_tensors.fpga_shape))
